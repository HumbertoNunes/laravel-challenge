...
function apply(job) {
    router.post(route('job.apply', {job: job.id, page: paginator.value.currentPage}, {
        preserveScroll: true,
    }))
}
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Opportunities</h2>
        </template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <OpenPosition :jobs="jobs.data">
                    <template #actions="{job}">
                        <CandidateJobActions v-if="profile.isCandidate" :job="job" @apply="apply"/>
